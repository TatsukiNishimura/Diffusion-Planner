...
def validate_model(model, val_loader, args, return_pred=False) -> tuple[float, float]:
    ...
    avg_loss_ego = total_loss_ego / total_samples_ego if total_samples_ego > 0 else 0.0
    ...
    if return_pred and predictions:
...
if __name__ == "__main__":
    ...
    # set up data loaders (WebDataset)
    with open(args.valid_set_list, "r") as f:
        valid_urls = json.load(f)
        
    # 1つのtarファイルあたりのサンプル数
    SAMPLES_PER_TAR = 1 
    TOTAL_VALID_SAMPLES = len(valid_urls) * SAMPLES_PER_TAR
    ...
    valid_loader = get_wds_loader(
        ...
        is_train=False, # 検証時はFalse
        total_samples=None,
        world_size=1
    )

    if global_rank == 0:
        print("Dataset Prepared: ~{} valid data ({} shards) \n".format(TOTAL_VALID_SAMPLES, len(valid_urls)))
    ...
    if hasattr(turn_indicators, 'shape'):
    ...
    if hasattr(predictions, 'shape') and predictions.dim() > 0:
